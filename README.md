...
uses to call run Julia code) only supports Julia v0.7+. If using WFVisual in
...
you'll have to setup WFVisual in Julia v0.7.
...
NOTE: This package uses Paraview as it's visualizer and it will call it with the
command `run('paraview')` after generating geometries in VTK format, so make
...
--user`). PyJulia by default will call whatever version of Julia it finds in the
system under the terminal command `julia`, and it only works with versions
higher or equal to v0.7, and since WFVisual was written in v0.6, the only
...
the folders `data/` for v0.6 and `datav07` for v0.7. Everytime a windfarm
